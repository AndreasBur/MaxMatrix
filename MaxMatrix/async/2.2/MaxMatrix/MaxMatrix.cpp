...
stdReturnType MaxMatrix::setRow(byte Row, const rowType* Value)
{
	...
	if(Orientation == MAXMATRIX_MODULE_ORIENTATION_90) {
		for(int Module = 0; Module < MAXMATRIX_NUMBER_OF_MODULES; Module++) {
			setColumnLL(Module, MAXMATRIX_COLUMN_NUMBER_OF_MODULE - Row - 1, (*Value)[Module]);
		}
	}
	...
	if(Orientation == MAXMATRIX_MODULE_ORIENTATION_270) {
		...
		for(int Module = 0; Module < MAXMATRIX_NUMBER_OF_MODULES; Module++) {
			setColumnLL(Module, Row, ValueReversed[Module]);
		}
	}
...
{
    if(sString != NULL) {
        //clear(); // TODO: clear or add character?
        SpriteShiftCounter = 0;
        String = sString;
 ...
 *  \return         -
 *****************************************************************************************************************************************************/
stdReturnType MaxMatrix::getColumnLL(byte Column, byte* Value)
{
 ...
 *  \return         -
 *****************************************************************************************************************************************************/
stdReturnType MaxMatrix::getColumnLL(byte Module, byte Column, byte* Value)
{
...
stdReturnType MaxMatrix::getRowLL(byte Module, byte Row, byte* Value)
{
	...
	{	// überprüfen!!!
		bitWrite(*Value, MAXMATRIX_COLUMN_NUMBER_OF_MODULE - Column - 1, bitRead(MatrixBuffer[Module * MAXMATRIX_COLUMN_NUMBER_OF_MODULE + Column], Row));
	}
} /* getRowLL */
...
{
	for(byte Module = 0; Module < MAXMATRIX_NUMBER_OF_MODULES; Module++) getRowLL(Module, Row, &(*Value)[Module]);
} /* getRowLL */
